...
The patch will be applied using intelligent pattern matching to the memory repository.""",
    ...
    search_replace: Optional[List[Dict[str, Any]]] = Field(
        default=None,
        description="""Search and replace operations with optional line range.

Format: [{"file": "main.py", "search": "old_code", "replace": "new_code", "start_line": 10, "end_line": 20}]

- file: Target file name (required)
...
Line range prevents unwanted replacements in other parts of the file.""",
    )
    ...
    commits: Optional[List[Dict[str, Any]]] = Field(
        default=None,
        description="""Create multiple commits with logical grouping.

Format: [{"files": ["file1.py"], "message": "Fix bug A", "description": "Details"}, {"files": ["file2.py"], "message": "Add feature B"}]

- files: List of files for this commit (required)
- message: Commit message (required)
- description: Optional detailed description

When provided, creates multiple commits on the same branch instead of one large commit.
Each commit maintains proper git history and can group related changes.""",
    )
    validate_files: Optional[List[str]] = Field(
        default=None,
        description="Optional list of files to validate after patching. If not provided, validation is skipped.",
    )
    validation_level: str = Field(
        default="required",
        description="Validation level: 'required' (syntax/compile only), 'recommended' (includes linting), 'all' (includes tests)",
    )
            ...
            for i, result in enumerate(results[:10], 1):  # Limit to 10 results
                if result["type"] == "function":
                    ...
                    output.append(
                        ...
                        f"\n    📄 {result['file']}:{result['line_start']}-{result['line_end']}{class_info}{signature_info}"
                    )
                else:  # class
...
The tool will apply changes to the memory repository."""
        ...
        try:
            ...
            if auto_commit:
                memory_result = self._perform_memory_operations(
                    modified_files,
                    branch_name or f"patch/{timestamp}",
            ...
            for file_path in files:
                ...
                for old_pattern, new_pattern in replacements:
                    if old_pattern in content:
                        ...
                        print(
                            f"🔧 Pattern replaced in {file_path}: {old_pattern} → {new_pattern}"
                        )
                        changes_applied += 1
                    else:
                        ...
                        if old_stripped and old_stripped in content:
                            ...
                            for i, line in enumerate(lines):
                                if old_stripped in line.strip():
                                    ...
                                    content = "\n".join(lines)
                                    print(
                                        f"🔧 Flexible pattern replaced in {file_path}: {old_stripped}"
                                    )
                                    changes_applied += 1
                                    break
    ...
    def _apply_search_replace(
        ...
        try:
            ...
            if search_text not in content:
                return f"❌ {file_path}: Search text not found: '{search_text}'"
    ...
    def _apply_search_replace(self, analyzer, search_replace_ops, auto_apply=False):
        """Apply search and replace operations with optional line range"""
        changes_made = []

        for op in search_replace_ops:
            file_path = op["file"]
            ...
            if file_path not in analyzer.virtual_files:
                changes_made.append(f"❌ {file_path} - File not found in virtual files")
                continue
            ...
            if start_line is not None or end_line is not None:
                ...
                if search_text in search_content:
                    ...
                    lines[start_idx:end_idx] = new_search_content.split("\n")
                    new_content = "\n".join(lines)
                    ...
                    changes_made.append(
                        f"✅ {file_path}:{start_line or 1}-{end_line or len(lines)} - {search_text} → {replace_text}"
                    )
                else:
                    changes_made.append(
                        f"❌ {file_path}:{start_line or 1}-{end_line or len(lines)} - Search text not found: {search_text}"
                    )
            else:
                # Search entire file (warn about potential multiple replacements)
                if content.count(search_text) > 1:
                    changes_made.append(
                        f"⚠️  {file_path} - Multiple matches found for: {search_text} (replaced first occurrence)"
                    )
                ...
                if search_text in content:
                    ...
                    changes_made.append(
                        f"✅ {file_path} - {search_text} → {replace_text}"
                    )
                else:
                    changes_made.append(
                        f"❌ {file_path} - Search text not found: {search_text}"
                    )

        return changes_made

    def _apply_unified_diff(self, analyzer, patch_content, auto_apply=False):
        """Apply unified diff patch and return changes made"""
        changes_made = []

        try:
            # Parse the patch
            patch_info = self._parse_patch_content(patch_content)

            for file_path in patch_info.get("files_modified", []):
                if file_path not in analyzer.virtual_files:
                    changes_made.append(
                        f"❌ {file_path} - File not found in virtual files"
                    )
                ...
                if auto_apply:
                    result = self._apply_git_patch_to_virtual_files(
                        analyzer, [file_path], patch_content
                    )
                    if "successfully" in result:
                        changes_made.append(
                            f"✅ {file_path} - Unified diff applied successfully"
                        )
                    else:
                        changes_made.append(f"❌ {file_path} - {result}")
                else:
                    changes_made.append(
                        f"📋 {file_path} - Would apply unified diff changes"
                    )

        except Exception as e:
            changes_made.append(f"❌ Unified diff parsing failed: {str(e)}")

        return changes_made


class DulwichApplyPatchTool(BaseTool if AWS_AVAILABLE else object):
    """Tool for applying patches to the memory repository using search/replace or unified diff"""

    name: str = "apply_patch"
    description: str = """Apply patches to files using multiple approaches:

APPROACH 1 - SEARCH & REPLACE (recommended for simple changes):
- Use 'search_replace' parameter: [{"file": "main.py", "search": "old_code", "replace": "new_code", "start_line": 10, "end_line": 20}]
- start_line/end_line are optional but recommended to avoid unwanted changes

APPROACH 2 - UNIFIED DIFF (for complex changes):
- Use 'patch_content' parameter with standard unified diff format
- MUST use preview_code/grep_content to see actual file content first

APPROACH 3 - MULTIPLE COMMITS (for logical grouping):
- Use 'commits' parameter: [{"files": ["file1.py"], "message": "Fix bug A", "description": "Details"}, {"files": ["file2.py"], "message": "Add feature B"}]
- Creates multiple commits on the same branch
- Each commit can have its own message and grouped files

Both approaches update memory repository and return detailed change summary."""
    ...
    def _run(
        ...
        search_replace: List[Dict[str, Any]] = None,
        ...
        validate_files: Optional[List[str]] = None,
        validation_level: str = "required",
        **kwargs,
    ) -> str:
        """Apply patches with comprehensive analysis and optional Git operations"""
        from datetime import datetime
        ...
        # Validate inputs
        if not search_replace and not patch_content:
            return "❌ No changes specified. Provide either search_replace or patch_content."
        ...
        try:
            ...
            # Handle search_replace vs patch_content logic
            if search_replace:
                # Process search and replace operations
                changes_made = self._apply_search_replace(
                    analyzer, search_replace, auto_apply
                )
                patch_info = {
                    "files_modified": [op.get("file") for op in search_replace],
                    "description": f"Search and replace operations on {len(search_replace)} patterns",
                }
            elif patch_content:
                ...
                changes_made = self._apply_unified_diff(
                    analyzer, patch_content, auto_apply
                )
                ...
                # Neither search_replace nor patch_content provided
                return "❌ No changes specified. Provide either search_replace or patch_content."

            # Helper function for allowed branch prefixes
            def get_allowed_prefixes():
                return ["fix/", "patch/", "feature/", "hotfix/", "bugfix/"]

            # Generate branch name if not provided
            if create_branch and not branch_name:
                timestamp = datetime.now().strftime("%Y%m%d%H%M")
                branch_name = f"patch/apply-{timestamp}"

            # Branch prefix validation - ensure proper branch naming
            allowed_prefixes = get_allowed_prefixes()
            final_target_branch = branch_name if create_branch else target_branch

            # Check if branch has allowed prefix
            has_allowed_prefix = any(
                final_target_branch.startswith(prefix) for prefix in allowed_prefixes
            )

            if not has_allowed_prefix:
                error_msg = [
                    f"🏷️  BRANCH NAMING POLICY",
                    f"❌ Branch must use allowed prefix: '{final_target_branch}'",
                    f"✅ Allowed prefixes: {', '.join(allowed_prefixes)}",
                    "",
                    "💡 Solutions:",
                    "   • Use create_branch=True to auto-generate a proper branch name",
                    f"   • Specify branch_name with prefix: 'fix/my-changes-{datetime.now().strftime('%Y%m%d%H%M')}'",
                    "   • Proper prefixes help organize and categorize changes",
                    "",
                    f"🎯 Recommended branch names:",
                    f"   • fix/your-change-{datetime.now().strftime('%Y%m%d%H%M')}",
                    f"   • patch/apply-{datetime.now().strftime('%Y%m%d%H%M')}",
                ]
                return "\n".join(error_msg)
            ...
            if patch_content and len(patch_lines) > 0:
            ...
            if changes_made:
                output.extend(
                    [
                        ...
                        "📋 Changes Applied:"
                        if auto_apply
                        else "📋 Changes Would Be Applied:",
                    ]
                )
                for change in changes_made:
                    output.extend([f"   {change}"])

            # Run validation checks if files specified by LLM
            if auto_apply and validate_files:
                validation_results = self._run_validation_checks(
                    analyzer, validate_files, validation_level
                )
                output.extend(["", "🔍 Validation Results:"])
                output.extend(validation_results)

            if auto_apply and (patch_content or search_replace):
                # Apply Git operations for patches
                if patch_content or search_replace:
                    memory_result = self._apply_patch_to_memory_repository(
                        patch_content,
                        target_branch,
                        create_branch,
                        branch_name
                        or f"patch/apply-{datetime.now().strftime('%Y%m%d%H%M')}",
                        auto_commit,
                        auto_push,
                        commit_message or "Apply patch via Logan",
                        {
                            "files_modified": [op.get("file") for op in search_replace]
                            if search_replace
                            else []
                    ...
                    output.extend([f"{memory_result}"])

            # Check for failures in changes_made
            has_failures = any("❌" in change for change in changes_made)

            if has_failures:
                error_messages = [change for change in changes_made if "❌" in change]
                return (
                    f"❌ PATCH FAILED: {len(error_messages)} error(s) occurred:\n"
                    + "\n".join(error_messages)
                    + "\n\nFull output:\n"
                    + "\n".join(output)
                )
    ...
    def _apply_search_replace(self, analyzer, search_replace_ops, auto_apply=False):
        ...
        for op in search_replace_ops:
            ...
            if start_line is not None or end_line is not None:
                ...
                normalized_search_content = " ".join(search_content.split())

                if normalized_search_text in normalized_search_content:
                    # Use original exact match for replacement
                    if search_text in search_content:
                        new_search_content = search_content.replace(
                            search_text, replace_text, 1
                        )
                    else:
                        # Fallback: try to find the actual text pattern
                        import re

                        pattern = re.escape(search_text).replace(r"\ ", r"\s+")
                        new_search_content = re.sub(
                            pattern, replace_text, search_content, count=1
                        )
                    lines[start_idx:end_idx] = new_search_content.split("\n")
                    new_content = "\n".join(lines)
                    ...
                    # Track actual lines changed
                    actual_start = start_line or 1
                    actual_end = end_line or len(lines)

                    changes_made.append(
                        f"✅ {file_path}:{actual_start}-{actual_end} - {search_text} → {replace_text}"
                    )
                else:
                    changes_made.append(
                        f"❌ {file_path}:{start_line or 1}-{end_line or len(lines)} - Search text not found: {search_text}"
                ...
                if search_text in content:
                    ...
                    # For whole-file search, try to find the line number where replacement occurred
                    replaced_line = None
                    if search_text in content:
                        content_lines = content.split("\n")
                    ...
                    if replaced_line:
                        changes_made.append(
                            f"✅ {file_path}:L{replaced_line} - {search_text} → {replace_text}"
                        )
                    else:
                        changes_made.append(
                            f"✅ {file_path} - {search_text} → {replace_text}"
    ...
    def _parse_patch_content(self, patch_content):
        """Parse patch content to extract file information"""
        patch_info = {
            "files_modified": [],
            "description": "",
            "commit_message": "",
        ...
        try:
            ...
            for line in lines:
                ...
                if line.startswith("--- a/") or line.startswith("--- b/"):
                    ...
                    if (
                        filename != "/dev/null"
                        and filename not in patch_info["files_modified"]
                    ):
                        patch_info["files_modified"].append(filename)
                elif line.startswith("diff --git"):
                    # Extract from git diff header
                    parts = line.split()
                    if len(parts) >= 4:
                        filename = parts[3]
                        if filename.startswith("b/"):
                            filename = filename[2:]
                        if filename not in patch_info["files_modified"]:
                            patch_info["files_modified"].append(filename)
    ...
    def _apply_patch_to_memory_repository(
        ...
        try:
            ...
            # Branch prefix validation using same helper function
            def get_allowed_prefixes():
                return ["fix/", "patch/", "feature/", "hotfix/", "bugfix/"]

            allowed_prefixes = get_allowed_prefixes()
            final_branch = branch_name if create_branch else target_branch

            # Check if branch has allowed prefix
            has_allowed_prefix = any(
                final_branch.startswith(prefix) for prefix in allowed_prefixes
            )

            if not has_allowed_prefix:
                operations_log.append(
                    f"❌ BLOCKED: Branch must use allowed prefix: '{final_branch}'"
                )
                operations_log.append(
                    f"✅ Allowed prefixes: {', '.join(allowed_prefixes)}"
                )
                ...
                operations_log.append(
                    "🏷️  Proper prefixes help organize and categorize patch branches"
                )
                return "\n".join(operations_log)

            operations_log.append(f"🎯 Target branch: {final_branch} (valid prefix)")
            ...
            for file_path in files:
                ...
                for old_pattern, new_pattern in replacements:
                    if old_pattern in content:
                            ...
                            if key_parts:
                                ...
                                for i, line in enumerate(lines):
                                    ...
                                    if matching_parts >= min(len(key_parts), 2):
                                        ...
                                        content = "\n".join(lines)
                                        print(
                                            f"🔧 Keyword-based replacement in {file_path}: {line.strip()} → {new_pattern}"
                                        )
                                        changes_applied += 1
                                        pattern_found = True
                                        break
                        ...
                        if not pattern_found:
                            print(f"⚠️  Pattern not found in {file_path}: {old_pattern}")
    ...
    def _run_validation_checks(
        ...
        """Run validation checks on modified files"""
        try:
            # Try to import validation system
            validation_spec = importlib.util.find_spec("validation_system")
            if validation_spec is None:
                # Try local import
                validation_path = os.path.join(
                    os.path.dirname(__file__), "validation_system.py"
                )
                if not os.path.exists(validation_path):
                    return [
                        "⚠️ Validation system not available - skipping validation checks"
                    ]

                spec = importlib.util.spec_from_file_location(
                    "validation_system", validation_path
                )
                validation_system = importlib.util.module_from_spec(spec)
                spec.loader.exec_module(validation_system)
            else:
                import validation_system

            # Prepare file contents from virtual files
            contents = {}
            for file_path in files:
                if file_path in analyzer.virtual_files:
                    virtual_file = analyzer.virtual_files[file_path]
            ...
            # Determine validation scope based on level
            required_only = validation_level == "required"

            # Run validation
            engine = validation_system.ValidationEngine()
            results = {}
            ...
            for file_path in files:
                if file_path in contents:
                    summary = engine.validate_file(
                        file_path, contents[file_path], required_only
                    )
                    ...
                    if summary.total_checks > 0:
                        ...
                        status = "✅" if summary.overall_success else "❌"
                        output_lines.append(f"   📄 {file_path}: {status}")
                        ...
                        for result in summary.results:
                            ...
                            if result.warnings and validation_level != "required":
                                for warning in result.warnings[
                                    :2
                                ]:  # Limit to first 2 warnings
                                    output_lines.append(f"      ⚠️ {warning}")
                                if len(result.warnings) > 2:
                                    output_lines.append(
                                        f"      ... and {len(result.warnings) - 2} more warnings"
                                    )

            # Summary
            if total_files > 0:
                summary_icon = "✅" if total_failed == 0 else "❌"
                output_lines.insert(
                    0,
                    f"   {summary_icon} Validated {total_files} files: {total_passed} passed, {total_failed} failed",
                )
            else:
                output_lines.append("   ℹ️ No files require validation")

            return output_lines

        except Exception as e:
            return [f"   ⚠️ Validation check failed: {str(e)}"]
    ...
    def _perform_memory_operations(
        ...
        """Perform memory repository operations for patch application with multiple commits"""
        ...
        try:
            ...
            # Determine commits to create
            if commits:
                # Use provided commits structure
                commit_groups = commits
                operations_log.append(f"📦 Creating {len(commits)} commits")
            else:
                ...
                operations_log.append(
                    f"📦 Creating single commit with {len(files)} files"
                )

            # Track all blobs we'll need
            all_file_blobs = {}
            for file_path in files:
                if file_path in analyzer.virtual_files:
                    virtual_file = analyzer.virtual_files[file_path]
                    if hasattr(virtual_file, "content"):
                        ...
                        blob = Blob.from_string(file_content)
                        memory_repo.object_store.add_object(blob)
                        all_file_blobs[file_path.encode("utf-8")] = blob.id
                        operations_log.append(f"📄 Added blob for: {file_path}")

            # Get base branch tree to preserve existing files in all commits
            base_tree_items = {}
            base_commit_id = None
            try:
                # Fetch base branch commit and tree
                from dulwich.client import get_transport_and_path

                push_url = analyzer.repo_info.url
                if hasattr(analyzer, "username") and hasattr(analyzer, "password"):
                    auth_username = analyzer.username
                    auth_password = analyzer.password
                    if auth_username and auth_password:
                        push_url = push_url.replace(
                            "https://", f"https://{auth_username}:{auth_password}@"
                        )

                client, path = get_transport_and_path(push_url)
                remote_refs = client.get_refs(path)

                # Check if target branch already exists, use its HEAD instead of base branch
                target_ref = f"refs/heads/{branch_name}".encode()
                base_ref = f"refs/heads/{analyzer.repo_info.branch}".encode()

                if target_ref in remote_refs.refs:
                    # Target branch exists, use its HEAD as parent for sequential commits
                    base_commit_id = remote_refs.refs[target_ref]
                    operations_log.append(
                        f"🔍 Target branch exists, using its HEAD: {base_commit_id.hex()[:8]}"
                    )
                elif base_ref in remote_refs.refs:
                    # Target branch doesn't exist, use base branch as parent
                    base_commit_id = remote_refs.refs[base_ref]
                    operations_log.append(
                        f"🔍 New branch, using base commit: {base_commit_id.hex()[:8]}"
                    )
                else:
                    operations_log.append("⚠️ Neither target nor base branch found")
                    base_commit_id = None

                if base_commit_id:
                    # Fetch the parent commit and its tree
                    client.fetch(path, memory_repo, lambda refs: [base_commit_id])

                    base_commit = memory_repo.get_object(base_commit_id)
                    base_tree = memory_repo.get_object(base_commit.tree)

                    # Store base tree items for reuse
                    for name, mode, sha in base_tree.items():
                        base_tree_items[name] = (mode, sha)

                    operations_log.append(
                        f"📁 Found {len(base_tree_items)} files in base branch"
                    )
                else:
                    operations_log.append(
                        f"⚠️ Base branch {analyzer.repo_info.branch} not found"
                    )

            except Exception as base_error:
                operations_log.append(f"⚠️ Could not fetch base branch: {base_error}")
            ...
            for i, commit_group in enumerate(commit_groups):
                ...
                # Add all base files first
                for name, (mode, sha) in base_tree_items.items():
                    tree.add(name, mode, sha)

                # Update with files from this commit group and all previous commits
                files_in_commit = []
                for prev_group in commit_groups[
                    : i + 1
                ]:  # Include all commits up to this one
                    for file_path in prev_group.get("files", []):
                        if file_path.encode("utf-8") in all_file_blobs:
                            tree.add(
                                file_path.encode("utf-8"),
                                0o100644,
                                all_file_blobs[file_path.encode("utf-8")],
                            )
                            if file_path in commit_group.get("files", []):
                                files_in_commit.append(file_path)

                memory_repo.object_store.add_object(tree)

                # Create commit
                commit_time = (
                    int(datetime.now(timezone.utc).timestamp()) + i
                )  # Slight time offset
                ...
                # Set parent
                if parent_commit_id:
                    commit.parents = [parent_commit_id]
                else:
                    commit.parents = []
                ...
                commit.encoding = b"UTF-8"
                commit.message = commit_group.get("message", f"Commit {i + 1}").encode(
                    "utf-8"
                )
                ...
                operations_log.append(
                    f"💾 Commit {i + 1}/{len(commit_groups)}: {commit_hash}"
                )
                operations_log.append(
                    f"   📝 {commit_group.get('message', f'Commit {i + 1}')}"
                )
                operations_log.append(f"   📄 Files: {', '.join(files_in_commit)}")

                commits_created.append(commit.id)
                parent_commit_id = commit.id

            # Use the last commit as the final commit for branch update
            final_commit = commit
            commit_hash = final_commit.id.decode("ascii")[:8]
            operations_log.append(f"🌿 Updated branch: {branch_name} -> {commit_hash}")
            ...
            if auto_push:
                try:
                    ...
                    if repo_url and real_push:
                        ...
                        operations_log.append(
                            "🚨 PERFORMING DIRECT MEMORY REPOSITORY PUSH!"
                        )
                        ...
                        try:
                            ...
                            operations_log.append(
                                "🔄 Pure memory repository push (no temp dirs)..."
                            )
                            ...
                            # Setup authenticated URL for push
                            push_url = repo_url
                            if auth_username and auth_password:
                            ...
                            operations_log.append(f"🔗 Push URL: {repo_url}")
                            operations_log.append(f"🌿 Pushing branch: {branch_name}")

                            # Get transport client
                            client, path = get_transport_and_path(push_url)

                            # Fetch current remote refs to understand what's there
                            operations_log.append("🔍 Fetching remote refs...")
                            remote_refs = client.get_refs(path)
                            operations_log.append(
                                f"🔍 Found {len(remote_refs.refs)} remote refs"
                            )

                            # Determine what objects we need to send (all commits and their trees)
                            objects_to_send = set()
                            for commit_id in commits_created:
                                objects_to_send.add(commit_id)
                                commit_obj = memory_repo.get_object(commit_id)
                                objects_to_send.add(commit_obj.tree)

                            # Add all tree objects and blobs
                            try:
                                tree_obj = memory_repo.get_object(commit.tree)
                                if hasattr(tree_obj, "items"):
                                    for name, mode, sha in tree_obj.items():
                                        objects_to_send.add(sha)
                                        operations_log.append(
                                            f"📄 Will send: {name} -> {sha.hex()[:8]}"
                                        )
                            except Exception as tree_error:
                                operations_log.append(
                                    f"⚠️ Tree processing: {tree_error}"
                                )

                            operations_log.append(
                                f"📦 Sending {len(objects_to_send)} objects"
                            )

                            # Final branch prefix validation before push
                            allowed_prefixes = [
                                "fix/",
                                "patch/",
                                "feature/",
                            ...
                            has_allowed_prefix = any(
                                push_branch.startswith(prefix)
                                for prefix in allowed_prefixes
                            )
                            if not has_allowed_prefix:
                                operations_log.append(
                                    f"❌ PUSH BLOCKED: Invalid branch prefix '{push_branch}'"
                                )
                                operations_log.append(
                                    f"✅ Allowed prefixes: {', '.join(allowed_prefixes)}"
                                )
                            ...
                            # Create the new refs we want to push (pointing to final commit)
                            new_refs = {
                                f"refs/heads/{push_branch}".encode(): final_commit.id
                            }

                            operations_log.append(
                                f"🚀 Pushing to branch: {push_branch}"
                            )

                            # Generate pack data with proper callback signature
                            def determine_wants(refs):
                                # Return the refs we want to update
                                return new_refs

                            def generate_pack_data(have, want, progress=None, **kwargs):
                                operations_log.append(
                                    f"🔍 generate_pack_data called with have={have}, want={want}, kwargs={kwargs}"
                                )
                                # Return pack data generator
                                return memory_repo.generate_pack_data(
                                    have, list(objects_to_send), progress=progress
                                )

                            # Push using send_pack with proper callback signatures
                            operations_log.append("🚀 Executing push...")
                            result = client.send_pack(
                                path, determine_wants, generate_pack_data
                            )

                            operations_log.append(f"🔍 Push result: {result}")
                            operations_log.append(
                                f"✅ Successfully pushed to: {repo_url}"
                            )
                            operations_log.append(f"🌿 Branch: {branch_name}")
                            operations_log.append(f"📝 Final commit: {commit_hash}")
                            operations_log.append(
                                f"📊 Total commits pushed: {len(commits_created)}"
                            )
                            operations_log.append("🎉 Pure memory push completed!")

                        except Exception as real_push_error:
                            import traceback

                            operations_log.append(
                                f"❌ Direct memory push failed: {real_push_error}"
                            )
                            operations_log.append(
                                f"❌ Error type: {type(real_push_error).__name__}"
                            )
                            operations_log.append("❌ Full stack trace:")
                            ...
                            # Re-raise the exception instead of falling back to simulation
                            raise real_push_error
                    ...
                    operations_log.append(
                        f"❌ Push error type: {type(push_error).__name__}"
                    )
                    operations_log.append("❌ Push error stack trace:")
                    ...
                    # Return failure instead of continuing to success
                    return "\n".join(operations_log)
            ...
            operations_log.extend(
                [
                    "",
                    f"🎉 Memory operations completed successfully!",
                    f"📝 Final commit ID: {commit_hash}",
                    f"🌿 Branch: {branch_name}",
                    f"📊 Total commits created: {len(commits_created)}",
                    f"📊 Files modified: {len(all_file_blobs)}",
                ]
            )
...
class LoganAssistant:
        ...
        if AWS_AVAILABLE:
            ...
            if self.debug_mode:
                ...
                if hasattr(self, "analyzer") and self.analyzer:
                    virtual_files = getattr(self.analyzer, "virtual_files", {})
                    functions = getattr(self.analyzer, "functions", {})
                    classes = getattr(self.analyzer, "classes", {})
                    print(
                        f"🐛 [DEBUG] Repository loaded: {len(virtual_files)} files, {len(functions)} functions, {len(classes)} classes"
                    )
                else:
                    print(
                        f"🐛 [DEBUG] No repository loaded - tools may not work properly"
                    )
        else:
            self.tools = []
            if self.debug_mode:
                print(f"🐛 [DEBUG] AWS not available - no tools initialized")
    ...
    def _create_and_load_analyzer(self):
        ...
        try:
            ...
            if repo_url:
                ...
                # Load repository with progress indication
                import time
                from datetime import datetime

                start_time = time.time()

                # Set verbose mode on analyzer based on debug mode
                analyzer.verbose = self.debug_mode
                ...
                if success:
                    ...
                    file_count = len(getattr(analyzer, "files", {}))
                    print(
                        f"📊 Analysis ready: {func_count} functions, {class_count} classes, {file_count} files",
                        flush=True,
                    )
                else:
    ...
    def _fetch_specific_commit(self, analyzer, repo_url, commit_id):
        ...
        try:
            self.debug(f"🔧 Fetching specific commit: {commit_id[:8]}...")
            ...
            if self.debug_mode:
                ...
                class DebugCallbackHandler(BaseCallbackHandler):
                    ...
                    def on_chat_model_start(self, serialized, messages, **kwargs):
                        if self.parent_assistant.debug_mode:
                            print(
                                f"🐛 [DEBUG] Chat model start: {len(messages)} messages"
                            )
                            print(f"🐛 [DEBUG] Model: {serialized}")
                    ...
                    def on_llm_end(self, response, **kwargs):
                        ...
                        if self.parent_assistant.debug_mode:
                            print(f"🐛 [DEBUG] Complete LLM Response Structure:")
                            print(f"🐛 [DEBUG]   Response Type: {type(response)}")
                            print(
                                f"🐛 [DEBUG]   Response Dir: {[attr for attr in dir(response) if not attr.startswith('_')]}"
                            )
                            print(f"🐛 [DEBUG]   Response: {response}")
                            print(f"🐛 [DEBUG]   Kwargs: {kwargs}")

                        # Capture the complete response for tool handling
                        if hasattr(response, "generations") and response.generations:
                            if self.parent_assistant.debug_mode:
                                print(
                                    f"🐛 [DEBUG] Found {len(response.generations)} generations"
                                )
                            for i, gen in enumerate(response.generations):
                                if self.parent_assistant.debug_mode:
                                    print(f"🐛 [DEBUG] Generation {i}: {gen}")
                                    print(
                                        f"🐛 [DEBUG] Generation {i} type: {type(gen)}"
                                    )
                                    print(
                                        f"🐛 [DEBUG] Generation {i} length: {len(gen) if hasattr(gen, '__len__') else 'N/A'}"
                                    )
                                for j, choice in enumerate(gen):
                                    if self.parent_assistant.debug_mode:
                                        print(f"🐛 [DEBUG] Choice {j}: {choice}")
                                        print(
                                            f"🐛 [DEBUG] Choice {j} type: {type(choice)}"
                                        )
                                        print(
                                            f"🐛 [DEBUG] Choice {j} dir: {[attr for attr in dir(choice) if not attr.startswith('_')]}"
                                        )
                                    if hasattr(choice, "message"):
                                        ...
                                        if self.parent_assistant.debug_mode:
                                            ...
                                            print(f"🐛 [DEBUG]   Type: {type(message)}")
                                            if hasattr(message, "content"):
                                                ...
                                                print(
                                                    f"🐛 [DEBUG]   Content: {content_preview}"
                                                )
                                            if hasattr(message, "tool_calls"):
                                                print(
                                                    f"🐛 [DEBUG]   Tool calls: {message.tool_calls}"
                                                )
                                            if hasattr(message, "response_metadata"):
                                                print(
                                                    f"🐛 [DEBUG]   Response metadata: {message.response_metadata}"
                                                )
                                            print(f"🐛 [DEBUG] End raw response\n")

                                        # Store response for main loop to handle
                                        self.parent_assistant.captured_response = (
                                            message
                                        )
                                    else:
                                        if self.parent_assistant.debug_mode:
                                            print(
                                                f"🐛 [DEBUG] No message found in choice"
                                            )
                        else:
                            if self.parent_assistant.debug_mode:
                                print(f"🐛 [DEBUG] No generations found in response")
                ...
                llm = llm.with_config(callbacks=[DebugCallbackHandler(self)])

            # Bind tools to the LLM
            if self.tools:
                if self.debug_mode:
                    print(f"🐛 [DEBUG] Binding {len(self.tools)} tools to LLM:")
                    for i, tool in enumerate(self.tools):
                        print(
                            f"🐛 [DEBUG]   Tool {i}: {tool.name} - {tool.description}"
                        )
                llm = llm.bind_tools(self.tools)
                if self.debug_mode:
                    print(f"🐛 [DEBUG] Tools successfully bound to LLM")
                    print(f"🐛 [DEBUG] LLM after binding: {type(llm)}")

                    # Check if the LLM has tool information
                    if hasattr(llm, "bound_tools"):
                        print(
                            f"🐛 [DEBUG] Bound tools count: {len(llm.bound_tools) if llm.bound_tools else 0}"
                        )
                    if hasattr(llm, "kwargs") and "tools" in llm.kwargs:
                        print(f"🐛 [DEBUG] Tools in kwargs: {len(llm.kwargs['tools'])}")
            else:
                if self.debug_mode:
                    print(f"🐛 [DEBUG] No tools available to bind")
    ...
    async def stream_response(self, user_input: str) -> None:
        ...
        try:
            ...
            if self.max_iterations > 1:
                for iteration in range(self.max_iterations):
                    ...
                    if self.debug_mode:
                        print(
                            f"🐛 [DEBUG] Starting stream with {len(messages)} messages"
                        )
                        print(f"🐛 [DEBUG] LLM type: {type(self.llm)}")
                        print(
                            f"🐛 [DEBUG] Last message: {messages[-1].content[:100] if messages else 'None'}"
                        )
                    ...
                    # Get response from callback handler
                    response = getattr(self, "captured_response", None)
                    ...
                    if (
                        ...
                        if self.debug_mode:
                            print(
                                f"\n🐛 [DEBUG] Found {len(response.tool_calls)} tool calls"
                            )
                            for i, tc in enumerate(response.tool_calls):
                                print(f"🐛 [DEBUG] Tool call {i}: {tc}")
                        ...
                        # Execute tools and add ToolMessage responses
                        for tool_call in response.tool_calls:
                            ...
                            # ALWAYS add ToolMessage to conversation (AWS Bedrock requires this)
                            try:
                                ...
                                if self.debug_mode:
                                    print(
                                        f"🐛 [DEBUG] Could not extract tool_call_id: {e}"
                                    )
                                fallback_id = f"tool_{len(tool_results)}"
                                messages.append(
                                    ToolMessage(
                                        content=result, tool_call_id=fallback_id
                                    )
                                )

                    # Determine if LLM wants to stop
                    llm_wants_to_stop = (
                        self._check_llm_stop_reason(response) if response else True
                    )
                    ...
                    token_info = (
                        ...
                        if response
                        else {"total_tokens": 0, "input_tokens": 0, "output_tokens": 0}
                    )

                    # Clear captured response for next iteration
                    ...
                    should_continue = (
                        ...
                        )  # Continue if tools used OR LLM hasn't signaled completion
                    )
                    ...
                    if should_continue:
                        ...
                        if should_trim:
                            ...
                            # Keep system message and trim back to last user message
                            system_msg = messages[0]

                            # Find the last user message (role='user')
                            last_user_idx = -1
                            for i in range(
                                len(messages) - 1, 0, -1
                            ...
                            if last_user_idx > 0:
                                # Keep system message + everything from last user message onward
                                messages = [system_msg] + messages[last_user_idx:]
                                print(
                                    f"🗂️  Context trimmed to {len(messages)} messages (kept from last user message)"
                                )
                            else:
                                # Fallback to original logic if no user message found
                                recent_messages = messages[-12:]
                                messages = [system_msg] + recent_messages
                                print(
                                    f"🗂️  Context trimmed to {len(messages)} messages (fallback)"
                                )

                        # Only add follow-up if no tools were executed (tools already added proper messages)
                        if not tool_results:
                        ...
                        total_context_tokens = sum(
                            len(str(msg.content).split()) * 1.3  # Rough token estimate
                            for msg in messages
                        ...
                        print(
                            f"\n🔄 Continuing to next iteration (tools used: {len(tool_results)}, context: {len(messages)} msgs, ~{int(total_context_tokens / 1000)}k context tokens{token_display}, total: {total_tokens_used} tokens)",
                            flush=True,
                        )
                    else:
                        ...
                        if not tool_results:
                            ...
                            print(
                                f"📊 Final token usage: {self.total_input_tokens} input + {self.total_output_tokens} output = {self.total_input_tokens + self.total_output_tokens} total tokens"
                            )
                        else:
                            print(
                ...
                if self.total_input_tokens > 0 or self.total_output_tokens > 0:
                    ...
                    if len(self.iteration_token_history) > 1:
                        print(f"   Iterations:    {len(self.iteration_token_history)}")
                        avg_input = sum(
                            h.get("input_tokens", 0)
                            for h in self.iteration_token_history
                        ) / len(self.iteration_token_history)
                        avg_output = sum(
                            h.get("output_tokens", 0)
                            for h in self.iteration_token_history
                        ) / len(self.iteration_token_history)
                        print(
                            f"   Avg per iter:  {avg_input:.0f} input + {avg_output:.0f} output = {avg_input + avg_output:.0f} total"
                        )
        ...
        # Show final token summary for single iteration mode
        if hasattr(self, "total_input_tokens") and (
            self.total_input_tokens > 0 or self.total_output_tokens > 0
        ):
            print(
                f"📊 Token usage: {self.total_input_tokens} input + {self.total_output_tokens} output = {self.total_input_tokens + self.total_output_tokens} total"
            )
        ...
        try:
            ...
            if hasattr(response, "stop_reason"):
                ...
                if stop_reason == "end_turn":
                    ...
                    return True
                elif stop_reason in ["tool_use", "tool_call"]:
                    # Safety check: if LLM says it wants tools but no tool_calls exist, stop
                    if not (hasattr(response, "tool_calls") and response.tool_calls):
                        if self.debug_mode:
                            print(
                                f"🐛 [DEBUG] LLM stop_reason={stop_reason} but no tool_calls found - stopping to prevent loop"
                            )
                        return True
                ...
                if isinstance(metadata, dict) and "stop_reason" in metadata:
                    ...
                    if stop_reason == "end_turn":
                        ...
                        if not (
                            ...
                            if self.debug_mode:
                                print(
                                    f"🐛 [DEBUG] Metadata stop_reason={stop_reason} but no tool_calls found - stopping to prevent loop"
                                )
                            return True
                        return False
            ...
            # Default: assume LLM wants to stop if no clear indication to continue
            return True

        except Exception:
            # If we can't determine stop reason, assume completion to prevent infinite loops
            return True

    def _check_task_drift(self, current_content: str) -> str:
        """Removed task drift detection - no longer needed with token-based context management"""
        return ""
    ...
    def _get_tool_call_id(self, tool_call):
        ...
        if hasattr(tool_call, "id") and tool_call.id:
            return tool_call.id
        elif isinstance(tool_call, dict) and "id" in tool_call and tool_call["id"]:
            return tool_call["id"]
        else:
    ...
    async def _execute_tool_call(self, tool_call):
        ...
        if self.debug_mode:
            ...
            print(f"🐛 [DEBUG] Tool call attributes: {dir(tool_call)}")
        ...
        try:
            ...
            if isinstance(tool_call, dict):
                ...
                if self.debug_mode:
                    print(
                        f"🐛 [DEBUG] Dict format - name: '{tool_name}', args: {tool_args}"
                    )
                ...
                if self.debug_mode:
                    print(
                        f"🐛 [DEBUG] Object format - name: '{tool_name}', args: {tool_args}"
                    )
            else:
                if self.debug_mode:
                    print(
                        f"🐛 [DEBUG] Unknown tool call format. Available attributes: {[attr for attr in dir(tool_call) if not attr.startswith('_')]}"
                    )
                return f"❌ Invalid tool call format: {type(tool_call)}"

            if not tool_name:
                if self.debug_mode:
                    print(f"🐛 [DEBUG] Empty tool name. Raw tool_call: {tool_call}")
                    print(
                        f"🐛 [DEBUG] Tool call keys (if dict): {list(tool_call.keys()) if isinstance(tool_call, dict) else 'N/A'}"
                    )
